...
class GoogLeNet(object):
    ...
    def __init__(self, args):
        ...
        pool5_7_7 = dropout(pool5_7_7, 0.7)
        ...
        try:
            ...
            self.model.load('.\param\GoogLenet\model_save.model')
        except:
            pass
        self.output = tf.argmax(self.network,1)
        # 置信度分布
        self.output2 = tf.nn.softmax(self.network)
    ...
    def predict_label(self, X):
        # return self.session.run(self.output, feed_dict={self.input: X}), self.session.run(self.output2, feed_dict={self.input: X})
        return self.session.run(self.output, feed_dict={self.input: X})

    def predict_label2(self, X):
        # return self.session.run(self.output, feed_dict={self.input: X}), self.session.run(self.output2, feed_dict={self.input: X})
        return self.session.run(self.output2, feed_dict={self.input: X})
